                                     ...
                                     G4TouchableHistory*)
{
	...
	if(aStep->IsLastStepInVolume()){
		G4StepPoint* pPost=aStep->GetPostStepPoint();
		if(pPost->GetTouchableHandle()->GetVolume()->GetName()=="Foil")exit_flag++;
	}

	if (edep==0. || TrackID!=1 ||exit_flag>0) return false;  //only primary particle
